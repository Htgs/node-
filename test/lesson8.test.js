...
describe('test/lesson8.test.js', function () {
    ...
    // 抽象请求测试
    const fibTest = function (n, statusCode, expect, done) {
        request.get('/fib')
            .query({n: n})
            .expect(statusCode)
            .end(function (err, res) {
    ...
    it('当n为0，结果为0', function (done) {
        fibTest(0, 200, '0', done);
    });
    it('当n为1，结果为1', function (done) {
        fibTest(1, 200, '1', done);
    });
    it('当n小于0时报错', function (done) {
        fibTest(-1, 500, 'n不能小于0', done);
    });
    it('当n大于10时报错', function (done) {
        fibTest(11, 500, 'n不能大于10,因为 Node.js 的计算性能没那么强', done);
    });
    it('当n不为数字时报错', function (done) {
        fibTest('aaa', 500, 'n必须为数字', done);
    });
    // 单独测试一下返回码 500
